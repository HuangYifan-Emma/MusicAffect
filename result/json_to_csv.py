...
def main():
    ...
    parser.add_argument('--time', '--parse-timestamp', action='store_true', default=False, help='parse the timestamp of filename and add the datetime to the output csv')
    parser.add_argument('--prefix', type=str, default='form_result_', help='set up the prefix of the file name')
    parser.add_argument('--index', '--include-index', action='store_true', default=False, help='generate id for each row of form data')
    ...
    parser.add_argument('-t', '--template', type=dir_file, help='set up the template json file for correct columns')
    ...
    if args.template:
        with open(args.template, 'r') as f:
            data = json.load(f)
            template_df = pd.DataFrame(columns=list(data.keys()))
            if args.time:
                template_df['submit_time'] = ''
            output_df = pd.concat([template_df, output_df], join='inner')
